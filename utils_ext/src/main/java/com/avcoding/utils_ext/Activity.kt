...
fun <T : Fragment> AppCompatActivity.addFragment(
    ...
    supportFragmentManager.commitTransaction(allowStateLoss) {
        add(container.id, fragmentClass, params.toBundle(), tag)
    }
}
...
fun <T : Fragment> AppCompatActivity.replaceFragment(
    ...
    supportFragmentManager.commitTransaction(allowStateLoss) {
        replace(container.id, fragmentClass, params.toBundle(), tag)
    }
}
...
fun <T : Fragment> AppCompatActivity.addFragmentToBackstack(
    ...
    supportFragmentManager.commitTransaction(allowStateLoss) {
        option?.invoke(this)
        replace(container.id, fragmentClass, params.toBundle(), tag).addToBackStack(tag)
    }
}
...
fun AppCompatActivity.validateBackPressed(onBackPressed: () -> Unit) {
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.R && supportFragmentManager.backStackEntryCount == 0) {
        if (isTaskRoot) {
            ...
            Log.e("InvalidBack","Application is potentially corrupted by an unknown activity")
            finishAffinity()
        }
    } else {
        onBackPressed()
